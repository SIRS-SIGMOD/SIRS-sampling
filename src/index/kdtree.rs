    ...
    pub fn get_top_level_alias(&self, query: &MBR) -> (usize, Option<AliasTable>, Vec<(usize, usize)>) {
        ...
        while !stack.is_empty() {
            ...
            if query.contains_mbr(&now.bounding_box) {
                candidates.push((now.start, now.end));
            } else {
                match &now.children {
                    None => {
                        candidates.push((now.start, now.end));
                        size += now.end - now.start;
                    }
                    Some((left, right)) => {
                        if query.intersects(&left.bounding_box) { stack.push(left); }
                        if query.intersects(&right.bounding_box) { stack.push(right); }
        ...
        let weights: Vec<f64> = candidates.iter().map(|(x, y)| (y - x) as f64).collect();
        if size > 0 { (size, Some(AliasTable::from(&weights)), candidates) }
        else { (size, None, candidates) }
    }
