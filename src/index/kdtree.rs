...
impl KDTreeNode {
        ...
        if len < KDTREE_THRESHOLD {
            ...
            *weight = weights.iter().sum();
            ...
            if level % 3 == 0 {
                ...
                let split = order_stat::kth_by(points, mid, |p1, p2| p1.p.z.partial_cmp(&p2.p.z).unwrap());
                left_bounding_box.high.z = split.p.z;
                right_bounding_box.low.z = split.p.z;
            }
    ...
    fn size(&self) -> usize {
        72 + if let Some((left, right)) = &self.children {
...
impl KDTree {
    ...
    pub fn size(&self) -> usize {
        &self.data.len() * 32 + self.root.size()
    }
