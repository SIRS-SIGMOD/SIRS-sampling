...
impl KDTree {
    pub fn from(mut data: Vec<WPoint>) -> KDTree {
        ...
        //let weights: Vec<f64> = data.iter().map(|p| p.weight).collect();
        let ialias = DyadicTree::from_func(&data, &|p: &WPoint| p.weight); 
        KDTree {
            root,
            data,
            ialias,
    ...
    pub fn size(&self) -> usize {
        &self.data.len() * 24 + self.root.size() + self.ialias.size()
    }
    ...
    pub fn range_sampling(&self, query: &MBR, k: usize) -> Vec<Point> {
        ...
        while samples.len() < k {
            ...
            let offset = self.ialias.single_sample(candidates[res].start, candidates[res].end);
            if query.contains(&self.data[offset].p) {
                samples.push(self.data[offset].p.clone());
            }
        }
