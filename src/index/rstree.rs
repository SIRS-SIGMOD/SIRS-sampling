...
impl RSTreeNode {
    ...
    fn from_data(data: &[WPoint], offset: usize) -> RSTreeNode {
        ...
        for p in data.iter() {
            minx = minx.min(p.p.x); miny = miny.min(p.p.y);
            maxx = maxx.max(p.p.x); maxy = maxy.max(p.p.y);
        }
...
impl RSTree {
    ...
    pub fn from(points: &mut [WPoint]) -> RSTree {
        ...
        let mut layout: Vec<WPoint> = Vec::new();
        ...
        let weights: Vec<f64> = layout.iter().map(|p| p.weight).collect();
        let ialias = DyadicTree::from(&weights);
    ...
    pub fn range_sampling(&self, query: &MBR, k: usize) -> Vec<Point> {
        ...
        while samples.len() < k {
            ...
            let offset = self.ialias.single_sample(candidates[res].offset, candidates[res].offset + candidates[res].size);
            if query.contains(&self.data[offset].p) {
                samples.push(self.data[offset].p.clone());
            }
        }
