...
impl ZVTreeNode {
    fn from(level: u8, high_bits: u64, data: &[(u64, f64)], start: usize, end: usize, weight: &mut f64) -> ZVTreeNode {
        if level == 32 || end - start <= MAX_ENTRIES_PER_NODE {
            *weight = data[start..end].iter().map(|p| p.1).sum();
            ZVTreeNode {
                children: None,
                weight: *weight,
                start,
...
impl ZVTree {
    pub fn from_zvpoints(mut data: Vec<(u64, f64)>) -> ZVTree {
        ...
        //let weights: Vec<f64> =  data.iter().map(|p| p.1).collect();
        let ialias = DyadicTree::from_func(&data, &|p: &(u64, f64)| p.1);
        ZVTree {
    ...
    pub fn size(&self) -> usize {
        self.data.len() * 16 + self.root.size() + self.ialias.size()
    }
    ...
    pub fn range_sampling(&self, query: &MBR, k: usize) -> Vec<Point> {
        ...
        while samples.len() < k {
            ...
            let offset = self.ialias.single_sample(unsafe { (*candidates[res]).start}, unsafe { (*candidates[res]).end } );
