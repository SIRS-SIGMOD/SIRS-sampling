...
fn main() -> std::io::Result<()> {
    ...
    let mut kdbtree = KDBTree::from(&data);
    let mut rsbtree = RSBTree::from(&data);
    ...
    {
        let now = Instant::now(); 
        let samples = kdbtree.range_sampling(&query, 100000);
    ...
    {
        let now = Instant::now(); 
        let samples = rsbtree.range_sampling(&query, 100000);
        println!("RSB-Sampling takes {} us", now.elapsed().as_micros());
