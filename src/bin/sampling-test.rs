...
fn main() -> std::io::Result<()> {
    ...
    lsmtree.print_levels();
    let kdtree = KDTree::from(&mut data);
    {
        let now = Instant::now(); 
        let samples = lsmtree.range_sampling(&query, 1000000);
        println!("LSM-Sampling takes {} us", now.elapsed().as_micros());
